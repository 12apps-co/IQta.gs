InstagramFetcherHandler = function() {
	...
	// Fetch tags containing search string (tagName) and their image count
	// DOESN'T HAZ TEST
	// DOESN'T TAKE CALLBACKS
	that.tagsSearch = function (options, cb) {
		// Make sure we got a tagName!
		check( options.tagName, String );

		var url = 'https://api.instagram.com/v1/tags/search?q='+options.tagName;

		that.log('--> --> searching tags containing: ' + options.tagName );

		return callEndpoint( url, null, options ); // callbacks will break it, therefore null
	}
